	...
	BEGIN
		UPDATE Clients
			...
			(CASE 
				...
				WHEN CONVERT(@field_to_update AS date) = client_added THEN CAST(@data_to_update AS date)
				WHEN @field_to_update = client_password THEN @data_to_update
				WHEN @field_to_update = client_com_code THEN @data_to_update
				END)
		PRINT('Updated !')
	END
	ELSE
...
EXEC Update_Client 1, 'client_firstname', 'New Jean-Louis'
